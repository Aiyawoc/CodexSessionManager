...
- age 1.3.1 — BSD-3-Clause. The official darwin-arm64 and windows-amd64 release archives are verified by the pinned SHA-256 and Sigsum proofs described in `packaging/age-v1.3.1.json` and `packaging/age-v1.3.1-windows-amd64.json` before bundling. Verification uses the hash-pinned canonical `sigsum-generic-2025-1` policy tracked in `packaging/sigsum-generic-2025-1.policy`.
