...
**Goal:** 先完成 v1.1 其它验收；验收完成后，下一优先级是敏感信息确定性 `Replace/Redact/Protect` 计划层，包括 diff、fingerprint 和风险控制。只有收到明确启动要求且官方能力/真实 round-trip 门禁满足后，才继续 Codex 桌面端 UI、app-only 执行、会话工作区、只读地图、恢复现场摘要、上下文分叉和来源追踪。
...
**Current gate (2026-09-01):** v1.1 继续完成除已关闭的 2.4 上下文应用以外的剩余验收。当前交付上下文审查/投影计划，以及任务盘点、备份、批量归档和反归档；原任务应用和派生投影不可用。二期 D1+ 未启动；未经明确要求不得开始。

**Current operation boundary:** 本计划遵循版本无关、契约敏感边界：版本、二进制和全量 schema 散列仅用于诊断与计划失效，不是归档授权条件；归档与反归档由静态、人工复核的最小操作契约逐项评估；无关变化自动兼容，相关方法/字段/关键枚举不兼容时只关闭受影响操作。当前 Codex 在线任务仅允许 `archive`/`unarchive` 执行；上下文审查与投影计划可用，但应用不可用；`purge`、restore/import 写入、trim/context apply 和 MCP 写入不可用。

标题修改/重命名不属于第一版范围且当前不可用；若在 v1.1 之后仍需研究，必须另行作出产品决策、定义受支持的操作契约并完成真实验收；本轮不启动 D1+。

## 全局约束

- D1 及之后的二期代码、插件写入能力和操作契约规则变更，必须等 v1.1 其它验收完成、敏感信息确定性修改优先方向完成必要规划，并收到明确启动要求后才能执行。本次文档整理不启动二期功能开发。
...
- 上下文审查与投影计划可用；应用到原任务不可用；派生投影必须在官方能力完成持久化、重启、后续模型可见和 reconcile 的完整 round-trip probe 后才能重新评估。
...
多 Board、别名、多父关系、团队共享、自动布局/WIP 规则、全账号自动导入、后台自动执行、自动归档和自动修改记忆不属于本二期。
...
- 二期执行闸门引用 v1.1 本机受控验收报告；未关闭前不得新增 UI 写入工具、操作契约例外或 schema 例外。
...
**D3 验收：**真实 Codex Desktop、相关操作契约评估通过的 App Server 能力和隔离测试账号中，一期可执行页面完成“盘点—计划—确认—执行—回读—审计”闭环；上下文页面在执行器未通过前只完成“审查—计划—源任务保护”闭环。重启/超时/部分成功不会重复写入；原任务、后代闭包、备份证据和记忆版本符合现有 CLI/PySide 结果。
...
- 任何失败不得把未完成操作报告为已归档、已反归档、已恢复或已修改。
...
- 清理：最终选择、完整后代闭包、独立备份、批量归档、取消归档和批量部分失败均可从回读与审计核对。
