...
- 永久删除：独立的最高风险流程，最后迁入，不得混入普通清理或自动化链路。
...
必须覆盖计划过期、计划/能力/内容指纹变化、后代闭包变化、账号根变化、未知字段、grant 过期、nonce 重放、跨 session 使用、重复幂等键、写前断线、写后响应丢失、批量部分成功、进程重启、备份损坏、错误 identity、恢复回读不一致、等待期不足、进程仍运行和确认挑战错误。
