...
- 缺少 ChatGPT/Skill 建议到本地 GUI 的统一审查请求协议；
- GUI 当前以 TrimReviewWindow 为中心，职责过多；
- 对话归档 GUI 缺少“备份并归档”闭环；
- Hook 保存计划后缺少待处理计划中心；
- 记忆管理尚未实现；
- 通用备份/恢复入口尚未进入主界面；
- LLM 建议尚未通过稳定 ID 与指纹绑定进入本地计划层。
...
- [ ] 将 `TrimReviewWindow` 拆为窗口壳与独立页面；
- [ ] 新增 `ConversationCleanupPage`；
- [ ] 新增 `ContextReviewPage`；
- [ ] 新增 `MemoryManagerPage` 占位与只读模式；
- [ ] 新增 `PendingPlansPage`；
- [ ] 新增 `BackupRestoreDialog`；
- [ ] 左侧功能栏提供五类入口；
- [ ] 保持 `csm trim review TASK_ID` 兼容；
- [ ] 增加 `csm gui open --request ...`；
- [x] 增加单实例和进程间请求转发。
...
- [ ] ReviewRequest/SuggestionBundle 已实现并经过测试；
- [ ] 桌面应用可按请求打开对话清理页；
- [ ] 建议对话按项目分组并可调整选择；
- [ ] 最终计划由本地重建；
- [ ] 归档前自动完成加密备份与验证；
...
1. 将本路线图加入 `docs/`；
2. 实现 `ReviewRequest` 与 `SuggestionBundle`；
3. 扩展 `AppPaths`，增加私有 requests/suggestions 目录；
4. 扩展 dispatcher，支持 `--request`；
5. 扩展 GUI 启动参数，先以现有窗口承接 `context_trim`；
6. 为 `conversation_cleanup` 和 `memory_edit` 增加只读占位页面或模式；
7. 编写请求伪造、过期、路径逃逸、账号根不匹配测试；
8. 再开始主窗口拆分和清理页完整闭环。
