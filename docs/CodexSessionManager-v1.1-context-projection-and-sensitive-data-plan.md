...
> 生效日期：2026-08-31
>
> 本文件是当前 v1.1 验收和后续研发顺序的规范性补充。在现有 README、验收 Runbook、Skill 文档或二期计划完成统一修订前，涉及上下文裁剪执行能力、2.4 判定和研发优先级的冲突表述以本文件、2.4 收口记录和 ADR 0009 为准。
...
这是独立执行层。当前基线没有通过真实 round-trip 验证的执行器，因此保持关闭。
...
保存上下文投影方案           可用
审查与导出投影               可用
应用到原任务                 当前 Codex 版本不支持
创建精简派生任务             运行时持久化探测未通过
等待受支持的执行能力         可用
```

禁用状态必须给出具体原因、精确 Codex 版本和最近一次能力探测结果，而不是笼统显示“操作失败”。
...
- 独立的 2.5 人工永久删除：按 ADR 0010 不设置固定等待期，但保留可信归档证据、archive-bound 当前备份、单根计划、进程复核和单次精确输入“确认删除”；
- 脱敏验收报告和已知限制。

2.4 上游阻塞不降低其它功能的通过标准，也不允许跳过永久删除门禁。
...
### `app_server.py` / `protocol_profiles.py`

- 将方法存在与语义能力分离；
- 记录 `0.142.1` 的派生注入负能力证据；
