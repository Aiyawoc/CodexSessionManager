...
验收期间**不得执行永久删除**。2.5 已因真实部分提交按 `CLOSED_WITH_UPSTREAM_BLOCKER` 关闭，GUI/CLI 只保留资格盘点、计划和审查。任何写请求超时或 JSON-RPC 错误都按“可能已经完成”处理：停止后续写入，先按目标 ID 重新读取实际状态，禁止盲目重试。
...
| 永久删除入口误触 | 只对单个已归档根开放；仍要求独立计划、可信归档证据、archive-bound 当前备份、进程复核和精确确认 |
