...
验收期间**不得执行永久删除**。永久删除当前没有资格盘点、计划、GUI、CLI、Skill、MCP 或执行器；2.5 历史部分提交仅作为关闭原因保留。任何写请求超时或 JSON-RPC 错误都按“可能已经完成”处理：停止后续写入，先按目标 ID 重新读取实际状态，禁止盲目重试。

本 Runbook 的 App Server 边界是版本无关、契约敏感：无关变化自动兼容；相关方法、字段或关键枚举不兼容时只关闭受影响操作。五项最小操作契约由静态规则和人工复核确定，版本、二进制和全量 schema 散列只用于诊断与计划失效。
...
只有相关操作契约、账号根、计划、备份、状态、内容指纹和完整后代闭包均通过复核时，才允许进入受支持的归档/反归档写入阶段。版本、二进制和全量 schema 散列只作为诊断与计划失效证据；相关契约缺失或账号根冲突均为 **NO-GO**。这不会开放上下文应用；2.4 的原任务应用不可用，派生投影须另有完整真实 round-trip 证据。
...
3. 本地安全补选默认不选中；
4. 永久删除不可用且不能混入归档；
5. “备份”首次只确认创建本机托管 age identity，后续不再输入 recipient 或选择 identity，并且始终先创建、完整复验备份；
...
8. 修改源内容、相关契约或计划文件后，旧计划进入 `INVALIDATED`；
...
| 永久删除入口误触 | 工具不存在；不得通过其它入口绕过当前不可用边界 |
