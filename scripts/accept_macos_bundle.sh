...
env PATH=/usr/bin:/bin:/usr/sbin:/sbin CSM_DATA_DIR="$CSM_ACCEPT_ROOT/data" \
  CSM_CONFIG_DIR="$CSM_ACCEPT_ROOT/config" CSM_CACHE_DIR="$CSM_ACCEPT_ROOT/cache" \
  CSM_LOG_DIR="$CSM_ACCEPT_ROOT/log" CSM_CODEX_HOME="$CSM_ACCEPT_ROOT/codex-home" \
...
env PATH=/usr/bin:/bin:/usr/sbin:/sbin CSM_GUI_SMOKE_EXIT_MS=750 QT_QPA_PLATFORM=offscreen \
  ...
  CSM_CODEX_HOME="$CSM_ACCEPT_ROOT/codex-home" \
