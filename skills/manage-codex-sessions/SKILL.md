...
7. 不自动永久删除。GUI 的独立只读分组只展示同时满足 CSM 可信归档证据、完整后代闭包和当前有效备份的已归档根候选，默认不选中，也不生成删除计划。固定 14 天等待已取消，但完整备份不能替代可信归档证据。只有用户明确要求并单选一个已归档根时，才允许由用户在 GUI 点击“删除”或显式运行 `csm purge plan`，并由用户本人提供精确 plan ID 和固定永久删除确认短语。
...
当前边界：上下文审查与投影计划可用；应用到原任务不可用；派生投影当前真实 round-trip 失败并保持阻塞。敏感信息的确定性 `Replace/Redact/Protect` 是后续优先方向，2.5 永久删除继续按独立单根计划、可信归档证据、archive-bound 当前备份、进程复核和双重确认门禁验收，不再设置固定等待期。
