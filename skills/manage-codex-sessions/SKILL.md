...
7. 不自动永久删除。GUI 的独立只读分组只展示同时满足 CSM 可信归档证据、完整后代闭包和当前有效备份的已归档根候选，默认不选中，也不生成删除计划。固定 14 天等待已取消，但完整备份不能替代可信归档证据。当前只允许用户显式运行 `csm purge plan` 审查单个已归档根；GUI 删除按钮和 `csm purge apply` 均因 2.5 上游 blocker 失败关闭。
...
当前边界：上下文审查与投影计划可用；应用到原任务不可用；派生投影当前真实 round-trip 失败并保持阻塞。敏感信息的确定性 `Replace/Redact/Protect` 是后续优先方向。2.5 永久删除真实写入已发生部分提交并按 `CLOSED_WITH_UPSTREAM_BLOCKER` 关闭；只允许资格盘点、计划和审查，不得通过 GUI、CLI、Skill 或 MCP 执行或重试残留 descendant。
